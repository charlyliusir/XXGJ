...
{
    /** 将图片缓存到本地*/
    NSString *imagefloder = [[self cacheStore] stringByAppendingPathComponent:floder];
    /** 创建文件夹*/
    if ([self createFilePath:imagefloder])
    {
...
- (void)startRecording {
    ...
    /** 自定义语音*/
    self.createDate = [self currentDuringTime];
    filename = [self getFilePath:@"audio" fileName:[NSString stringWithFormat:@"%@.spx", self.createDate]];
...
- (void)encapsulatingOver {
    if (self.delegate) {
        [self.delegate recordingFinishedWithFileName:self.createDate time:[self recordedTimeInterval]];
    }
}
