...
static CGFloat CELL_MARGIN = 4;
static CGFloat CELL_LINE_MARGIN = 4;
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = XX_RGBCOLOR_WITHOUTA(248, 248, 248);
    // 获取相册
    [self setupAssets];
...
- (void)addNavBarCancelButton{
    UIBarButtonItem *temporaryBarButtonItem  = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(cancelBtnTouched)];
...
- (void) setupToorBar{
    ...
    [self.previewBtn setTitleColor:XX_RGBCOLOR_WITHOUTA(153, 153, 153) forState:UIControlStateNormal];
    [self.previewBtn setTitleColor:[XX_RGBCOLOR_WITHOUTA(153, 153, 153) colorWithAlphaComponent:0.5] forState:UIControlStateDisabled];
    [self.previewBtn setEnabled:YES];
    [self.previewBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
    [self.previewBtn setTitle:@"预览" forState:UIControlStateNormal];
    [self.previewBtn addTarget:self action:@selector(previewBtnTouched) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.sendCountBtn addTarget:self action:@selector(sendBtnTouched) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.sendBtn addTarget:self action:@selector(sendBtnTouched) forControlEvents:UIControlEventTouchUpInside];
